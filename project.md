...
**Last updated:** June 24, 2026 (Sprints E + F — **Settings hub & devices** + **mobile header/status polish**: server-tracked **logged-in devices** with **remote logout** (sessions baked into the JWTs), a new **Settings** area (gear below the profile name; Security moved here) with a **Devices** screen; and on mobile a tighter **chat header** with **last seen** in the contact bar plus a **WhatsApp-style scroll-linked status strip**. Builds on Sprint D.2's sumo-libsodium backup fix + dismissible restore.)
...
- **Sprint F — Mobile chat header & WhatsApp status** ✅ **Done** — hide placeholder call buttons on phones so **name + last seen fit**; contact bar shows **Online / last seen** on mobile (encrypted pill hidden < sm); **scroll-linked status strip** under the header (`sm:hidden`) that hides on scroll-up and returns at the bottom; self chat uses your own status.
