...
### Sprint J — Notification Accept/Reject reliability 🔔✅
The bell stacked **duplicate friend-request rows** for one request, and accepting one left the others showing Accept/Reject — which then **400'd silently** ("Invalid friendship state"), so it took many clicks before hitting a row that worked.
...
- **Client (`NotificationCenter.tsx`):** a successful Accept/Reject/Block **purges all matching rows** from the cache (by `friendshipId`, or by `actor` for block) — not just the clicked one — and fixes the unread count; failures now show an **inline error** ("This request was already handled." etc.) instead of failing silently.
...
- **Sprint J — Notification Accept/Reject reliability** ✅ **Done** — **deduped** friend-request notifications (one row per `friendshipId`), **delete** stale rows on accept/reject/cancel, **purge all duplicate rows** on success client-side, and **surface errors inline** so it no longer takes many clicks to accept a request.
