...
- **G.3 — Floating status bubble (not a big box):** the mobile status went from a **full-width bar** to a small **left-aligned pill floating under the avatar** (`ConversationStatusBanner`: no full-width border/bg, `ml-11` indent past the back button, rounded `bg-surface-2` pill, `max-w-[calc(100vw-7rem)]`). Same **scroll-linked** show/hide, now with a subtle slide (`translate-y`).
