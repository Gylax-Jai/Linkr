...
**Last updated:** June 24, 2026 (**Phase 4 — Chat UX & account controls**: per-user **mute notifications** (silences the in-app `message` notification, with a muted icon in the list + an **Archived** section in the sidebar) and **archive chats**; a real **Privacy settings UI** (last seen & online / profile details / who can friend-request — segmented controls saving to `PATCH /api/users/me/privacy`); **forward a message to a friend** (E2EE-safe — the client decrypts the original and **re-encrypts for the new recipient**; media is copied by reference) plus **share contact** (native share sheet on mobile, clipboard fallback on desktop); **report user** (reason + optional details → server `Report` model, deduped per open report); and **account deletion** with two modes — **deactivate** (hidden + signed out everywhere, fully restored if you log back in within 15 days, otherwise a server cron **permanently purges** it) or **delete permanently now** (cascades across chats, messages, friendships, notifications, reports, sessions, OTPs). Both modes require typing your exact username/email to confirm, clear the refresh cookie, and drop you to the login screen; a deactivated account is reactivated on next Google login and blocked by auth middleware until then. Builds on Sprint 3.1.5 — **mobile audio routing UX**: the audio button **always opens a dropdown** on phone — **Earpiece + Speaker**, plus **Bluetooth** when connected; default without BT is **Earpiece** (not loudspeaker); outgoing calls **wait for mic permission** before ringing/signaling. Builds on Sprint 3.1.4 — **Bluetooth icon on mobile**: the audio-route control no longer shows a meaningless "Default" when sound is in your earbuds — it **always detects the connected peripheral** (even on Android Chrome / iOS Safari where the web can't switch sinks) and shows the right **Bluetooth / Headset / Speaker** icon; the caller's mic is now grabbed at call start so labels are unlocked and the icon is correct even during "Calling…". Builds on Sprint 3.1.3 — **call UX polish**: calls now **open full-screen first** and only **minimize to the top bar on the down-arrow or a tap outside** the call card; the speaker control became a real **audio-route dropdown** (Bluetooth / Headset / Speaker / Earpiece, with the active one checked) shown in both the overlay and the bar; and the route **icon now reflects the actually-connected device** — earbuds show a **Bluetooth** icon, not "Default" — by inferring the OS-default sink's true target from all device labels (inputs included). Builds on Sprint 3.1.2 — **call UX polish**: the minimized call is now a **WhatsApp-style full-width strip glued to the top** that **pushes content down** (no more floating card over the contact), calls **start minimized** in the background, **call logs hug the corners** (outgoing right / incoming left) like chat bubbles, and the speaker toggle became a **smart audio-route cycler** — outputs are classified into **Bluetooth / headset / speaker / earpiece**, auto-prioritized (Bluetooth > headset > speaker) and **re-routed mid-call** when earbuds connect. Builds on Sprint 3.1.1 — **call UX polish**: calls now **run in the background** (minimize to a slim top **call bar** and keep chatting), **mute + speaker** work in every phase (speaker via `setSinkId` where supported), there are **ringback/ringtone sounds** (Web-Audio, no asset files), the status shows **"Ringing…" when the callee is online vs "Calling…" when offline** (offline calls ring out to a missed entry after 35s), and every call now writes a **call-log row in the chat** — completed (with duration) / missed / declined / cancelled — shown as a WhatsApp-style pill and in the chat-list preview. Builds on Sprint 3.1 — **Phase 3 calls begin**: a quality-first **voice call** between accepted friends, end-to-end over **WebRTC** with media peer-to-peer — the server only relays **friendship-gated signaling** and mints **short-lived TURN credentials** (`GET /api/calls/ice-config`, coturn `use-auth-secret`; `TURN_SECRET` stays server-side). Audio is tuned for clarity — **48 kHz mono Opus + in-band FEC, ~64 kbps**. New `CallEngine`/`CallProvider` drive the lifecycle; an **incoming-call modal** + **in-call overlay** (status, live timer, mute, hang-up) ship the UI, and the header **Voice** button is now live (**Video** = coming soon, 3.2). Builds on Sprint J — **notification reliability + manage**: friend-request notifications are now **deduped** (one actionable row per request), **cleaned up** on accept/reject/cancel, with **instant optimistic** Accept/Reject/Block that purges duplicate rows and shows **inline errors** — so accepting works on the first click instead of needing many — plus a **Clear all** button and a **per-row delete icon**. Builds on Sprint I — **mobile soft-keyboard fix**: focusing the message box no longer scrolls the chat header off-screen — the header stays pinned and only the message list shrinks above the keyboard, via `interactive-widget=resizes-content` + a `visualViewport`-driven `--app-vh`. Builds on Sprint H — **full-screen mobile chat + overlay status**: the Linkr top bar now **hides on phones when a chat is open** so the conversation fills the screen like a native app, and the under-avatar status is a true **overlay bubble** (no empty strip / no "incoming message" look). Builds on Sprint G — **mobile header + Profile/Settings polish** on top of E + F: brought the **voice/video icons back** (compact on phones) and tightened header spacing so **last seen** stays readable; the mobile status is now a **floating left-aligned bubble under the avatar** (not a full-width bar); the **Settings gear was removed from the sidebar** (Settings lives in the user menu only); and **Profile ↔ Settings are fully separated** (no cross-links). Builds on Sprints E + F — Settings hub & devices + mobile header/status — and Sprint D.2's sumo-libsodium backup fix.)
...
- **4.2 (patch) — Stability, privacy enforcement & perf:** fixes from production testing — **privacy is now enforced end-to-end** (server strips `online`/`lastSeen`/bio/status when the viewer isn't allowed; profile visibility is **Friends / Nobody** only; presence broadcasts are suppressed for "Nobody"); client headers/sidebar/details **respect `presenceVisible` / `profileDetailsVisible`**; **Report modal portaled** to `document.body` so it isn't clipped by the header menu; **unread badge** clears reliably via **`PATCH /api/chat/:chatId/read`** (socket + HTTP) with list-cache patches instead of refetch races; **chat list load** batched (no per-row N+1 user/friendship queries); **calls** — pending incoming delivered on reconnect, 3‑min server TTL clears ghost sessions, **`CALL_END`/`CALL_REJECT` emitted on all local teardown paths**, active phase only after WebRTC `connected` (fixes yellow-dot/no-audio), socket reconnect on token refresh + tab visibility. **Also:** strangers see **contact cards by default** (avatar, name, @username) until the user sets profile privacy to **Nobody**; **Find friends** rows open a contact-card modal; **friend requests disabled** shows a clear popup (`REQUESTS_DISABLED`); **new users** get a **one-time E2EE security prompt** after onboarding recommending a recovery passphrase (Settings → Security).
