...
- **Calls no longer cut at ~3 min (`calls.socket.ts`):** the old `MAX_CALL_MS = 3 min` "safety TTL" started at **initiate** and force-ended every call — including connected ones — so the in-call timer cut out around **2:55–2:58** (3 min minus ring/setup). Replaced with two limits: **`RING_TIMEOUT_MS = 30s`** for unanswered calls and **`MAX_ACTIVE_CALL_MS = 2h`** for connected calls. The 2h ceiling now starts on **`CALL_ACCEPT`** (talk time), never from initiate; `pruneStaleCalls` measures accepted calls from `acceptedAt`. Net effect: **unanswered → ends at 30s; answered → runs until hang-up, hard cap 2h**. *(Render redeploy required.)*
- **Rear camera switch (mobile) (`CallEngine.ts`, `callConfig.ts`, `CallProvider.tsx`, `CallOverlay.tsx`, `call.store.ts`):** new **`switchCamera()`** captures the other camera (`facingMode` user↔environment) and swaps it on the live `RTCRtpSender` via **`replaceTrack`** (no renegotiation), re-applying the 720p bitrate. A **flip-camera** control appears in the video call controls on phones; the local PiP mirrors **only** for the front camera (`cameraFacing` in the store).
- **Typing flicker fixed (`SocketProvider.tsx`):** each `USER_TYPING` event used to schedule a **new** 3s clear without cancelling the previous one, so an old timer fired mid-typing and the header flipped **typing → online → typing**. Now one timer per chat (`Map<chatId, timeout>`), reset on each event (~4s), cleared on unmount.
- **Typing at the bottom (WhatsApp-style) (`ConversationPane.tsx`):** typing was removed from the header (which now shows a **stable presence line**) and a **`TypingIndicator`** ("<name> is typing…" with animated dots) renders just above the composer.
- **Image ⋮ menu — Download instead of Copy text (`ConversationPane.tsx`, `MessageMedia.tsx`):** the message menu now hides **Copy text** when there's no text and shows **Download** for images (image + caption shows both). New exported **`downloadMessageMedia(message)`** fetches authed (relative) media as a blob before saving.
- **Images open in-app (`MessageMedia.tsx`, `Lightbox.tsx`, `ui.store.ts`):** clicking a chat image opened a **new browser tab** (`window.open`); it now opens the global **Lightbox** with a new **`"chat"` variant** — full-frame `object-contain` viewer with a **Download** button and Escape-to-close — instead of leaving LINKr.
